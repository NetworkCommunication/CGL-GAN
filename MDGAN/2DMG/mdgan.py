...
class Server(threading.Thread):
    ...
    def run(self):
        ...
        opti = optim.Adam(net_g.parameters(), lr=self.lr_g, betas=(b1, b2))
        while t > 0:

            if t % num_plt == 0:
                self.plot_2d(net_g)

            self.train(net_g, opti)
    ...
    def train(self, net_g, opti):
        ...
        for client in self.client_list:
            workers[client].queen_d.put((self.idx, Xd.detach().clone()))
            workers[client].queen_g.put((self.idx, Xg.clone()))

        opti.zero_grad()
        loss = torch.zeros(num_workers)
...
class Worker(threading.Thread):
    ...
    def train(self, net_d, loss, opti_d):
        ...
        # with lock: print(self.name, "execute time:", end - start, "loss:", D_loss.item())
...
if __name__ == "__main__":
    ...
    for k in range(2, 3):
        ...
        SimulationName = time.strftime("%Y-%m-%d %H-%M-%S",
                                       time.localtime()) + "-MDGAN-" + "iid_%d" % iid + "-data_class_%d" % num_class + "-epoch_%d" % epoch + "-cloud_%d" % cloud_epoch
