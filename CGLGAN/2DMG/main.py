...
class Server(threading.Thread):
    ...
    def run(self):
        ...
        with open("./logger/" + SimulationName + "/config{}.pkl".format(self.name), 'wb') as f:  # 将数据写入pkl文件
             pkl.dump((self.client_list, self.beta, lambda_list, [workers[idx].test for idx in self.client_list], gen_data), f)
    ...
    def train(self, net_g, opti, N):
        ...
        # 计算权重和并更新神经网络
        gamma = F.softmax(self.Lambda * loss, dim=0).detach()
        ...
        self.Lambda = self.Lambda + 10 * grad
